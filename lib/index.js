...
async function fetchBrowserSessionItems(cookie, limit = 20) {
  ...
  while ((match = re.exec(html)) && items.length < limit) {
    ...
    seen.add(key)
    items.push({ source: '知乎会话', title, url, summary: '', author: '', time: '', likes: '' })
  }
...
const ZHIHU_QRCODE_URL = 'https://www.zhihu.com/api/v3/account/api/login/qrcode'
const ZHIHU_UDID_URL = 'https://www.zhihu.com/udid'
const ZHIHU_CAPTCHA_URL = 'https://www.zhihu.com/api/v3/oauth/captcha/v2?type=captcha_sign_in'
const ZHIHU_ME_URL = 'https://www.zhihu.com/api/v4/me'
const ZHIHU_WEB_ZSE93 = '101_3_3.0'
const ZHIHU_DESKTOP_USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36'
const qrAttempts = new Map()
const qrEventLog = []

function logQrEvent(attempt, event, details = {}) {
  const id = typeof attempt === 'string' ? attempt : attempt?.id
  const status = typeof attempt === 'string' ? '' : attempt?.status
  const entry = {
  ...
  qrEventLog.push(entry)
  while (qrEventLog.length > 200) qrEventLog.shift()
}

function publicQrLog() {
  return qrEventLog.slice(-80)
}
...
function zhihuSessionPath() {
  return join(dshHome(), 'zhihu-session.json')
}

function cookieHeaderFromMap(cookies) {
  return Object.entries(cookies || {}).filter(([, value]) => String(value || '') !== '').map(([name, value]) => `${name}=${value}`).join('; ')
}

function parseCookieAssignments(rawCookie) {
  const skip = new Set(['Domain', 'Path', 'Expires', 'Max-Age', 'HttpOnly', 'Secure', 'SameSite'])
  return String(rawCookie || '').split(';').map((item) => item.trim()).filter(Boolean).reduce((out, item) => {
    if (!item.includes('=')) return out
    const name = item.slice(0, item.indexOf('=')).trim()
    const value = item.slice(item.indexOf('=') + 1).trim()
    if (!name || !value || skip.has(name)) return out
    out[name] = value
    return out
  }, {})
}
...
function mergeResponseCookies(cookies, res) {
  for (const raw of setCookieHeaders(res)) Object.assign(cookies, parseCookieAssignments(raw))
}

function zhihuDesktopHeaders(referer = '') {
  const headers = {
...
function zhihuLoginHeaders(cookies, referer, polling = false) {
  const headers = {
    ...zhihuDesktopHeaders(referer),
    origin: ZHIHU_HOME_URL.replace(/\/$/, ''),
...
async function zhihuRequest(url, options, cookies) {
  const res = await fetch(url, options)
  mergeResponseCookies(cookies, res)
  return res
}
...
function zhihuPayloadData(payload) {
  return payload?.data || payload?.Data || payload
}

async function prefetchQrContext(cookies) {
  const signin = await zhihuRequest(ZHIHU_SIGNIN_URL, { headers: zhihuDesktopHeaders(ZHIHU_HOME_URL) }, cookies)
  mergeResponseCookies(cookies, signin)
  await zhihuRequest(ZHIHU_UDID_URL, { method: 'POST', headers: zhihuLoginHeaders(cookies, ZHIHU_SIGNIN_REFERER_URL), body: '{}' }, cookies).catch(() => null)
  await zhihuRequest(ZHIHU_CAPTCHA_URL, { headers: zhihuLoginHeaders(cookies, ZHIHU_SIGNIN_REFERER_URL) }, cookies).catch(() => null)
}

function normalizeQrDeadline(expiresAt) {
  const now = Date.now()
  const n = Number(expiresAt)
  if (!Number.isFinite(n) || n <= 0) return now + 120_000
  if (n < Math.floor(now / 1000)) return now + Math.min(n * 1000, 86_400_000)
  return n < 10_000_000_000 ? n * 1000 : n
}

function qrDataUrl(text) {
  return new Promise((resolve) => {
    execFile('qrencode', ['-t', 'SVG', '-o', '-', text], { maxBuffer: 1024 * 1024 }, (error, stdout) => {
      if (error) { resolve(''); return }
      resolve(`data:image/svg+xml;base64,${Buffer.from(stdout).toString('base64')}`)
    })
  })
}

function readZhihuSession() {
  try {
    const session = JSON.parse(readFileSync(zhihuSessionPath(), 'utf8'))
    if (session?.version === 1 && session.cookies && typeof session.cookies === 'object') return session
  } catch { /* no persisted Zhihu session */ }
  return null
}

function saveZhihuSession(cookies, source = 'qr-login') {
  mkdirSync(dshHome(), { recursive: true })
  const session = { version: 1, source, savedAt: Date.now(), cookies, userAgent: ZHIHU_DESKTOP_USER_AGENT }
  writeFileSync(zhihuSessionPath(), JSON.stringify(session), { mode: 0o600 })
  return session
}

function clearZhihuSession() {
  try { rmSync(zhihuSessionPath(), { force: true }) } catch { /* session cleanup is best-effort */ }
}

function publicSessionStatus(session, verification = null) {
  ...
  return {
    ...
    verified: verification?.ok === true ? Boolean(verification.loggedIn) : undefined,
    verifyStatus: verification?.status || 0,
  }
}

async function sessionStatus(verify = false) {
  const session = readZhihuSession()
  if (!session) return publicSessionStatus(null)
  if (!verify) return publicSessionStatus(session)
  const verification = await verifyZhihuCookie(cookieHeaderFromMap(session.cookies))
  return publicSessionStatus(session, verification)
}

async function startQrLogin(baseAttemptId = '') {
  const id = randomUUID()
  const baseAttempt = baseAttemptId ? qrAttempts.get(baseAttemptId) : null
  const cookies = baseAttempt?.cookies ? { ...baseAttempt.cookies } : {}
  const attempt = { id, cookies, status: 'starting', message: '正在初始化扫码登录', createdAt: Date.now(), updatedAt: Date.now(), deadline: Date.now() + 120_000 }
  qrAttempts.set(id, attempt)
  logQrEvent(attempt, 'start', { inherited: Boolean(baseAttempt), cookieCount: Object.keys(cookies).length, hasAuthCookie: Boolean(cookies.z_c0) })
  try {
    await prefetchQrContext(cookies)
    const res = await zhihuRequest(ZHIHU_QRCODE_URL, { method: 'POST', headers: zhihuLoginHeaders(cookies, ZHIHU_SIGNIN_REFERER_URL), body: '{}' }, cookies)
    const raw = await readJsonSafe(res)
    const data = zhihuPayloadData(raw)
    const token = data.token || data.qrcode_token || data.qrcodeToken
    const link = data.link
    if (res.status >= 400 || !token || !link) throw new Error(`二维码获取失败：HTTP ${res.status}`)
    attempt.status = 'waiting'
    attempt.message = '请使用知乎 App 扫描二维码'
    attempt.token = token
    attempt.link = link
    attempt.deadline = normalizeQrDeadline(data.expires_at ?? data.expiresAt)
    attempt.updatedAt = Date.now()
    attempt.qrSvgDataUrl = await qrDataUrl(link)
    logQrEvent(attempt, 'qr-ready', { httpStatus: res.status, cookieCount: Object.keys(cookies).length, hasAuthCookie: Boolean(cookies.z_c0), hasQrSvg: Boolean(attempt.qrSvgDataUrl) })
    return publicQrAttempt(attempt)
  } catch (error) {
    attempt.status = 'failed'
    attempt.message = error instanceof Error ? error.message : String(error)
    attempt.updatedAt = Date.now()
    logQrEvent(attempt, 'start-failed', { message: attempt.message })
    return publicQrAttempt(attempt)
  }
}

function syncCookiesFromQrInfo(cookies, info) {
  Object.assign(cookies, parseCookieAssignments([info?.cookie, info?.cookies].filter(Boolean).join(';')))
  if (info?.z_c0) cookies.z_c0 = info.z_c0
  if (info?.zC0) cookies.z_c0 = info.zC0
  if (info?.z_c0 && !cookies.z_c0) cookies.z_c0 = info.z_c0
}

function qrDebugInfo(info, cookies) {
  const loginStatus = String(info?.login_status || info?.loginStatus || '')
  return {
    httpStatus: Number(info?._httpStatus || 0),
    scanStatus: info?.status ?? null,
    loginStatus,
    hasSuccessFlag: Boolean(info?.success || info?.logged_in || info?.loggedIn),
    hasUserId: Boolean(info?.user_id || info?.userId),
    hasAccessToken: Boolean(info?.access_token || info?.accessToken),
    hasAuthCookie: Boolean(cookies?.z_c0),
    cookieNames: Object.keys(cookies || {}).filter((name) => !/^z_c0$/i.test(name)).slice(0, 8),
  }
}

async function collectPostLoginCookies(cookies) {
  await zhihuRequest(ZHIHU_HOME_URL, { headers: zhihuLoginHeaders(cookies, ZHIHU_SIGNIN_URL, true) }, cookies).catch(() => null)
  await zhihuRequest(ZHIHU_ME_URL, { headers: zhihuLoginHeaders(cookies, ZHIHU_SIGNIN_URL, true) }, cookies).catch(() => null)
}

function isQrSuccess(info, cookies) {
  const loginStatus = String(info?.login_status || info?.loginStatus || '').toUpperCase()
  return Boolean(info?.user_id || info?.userId || info?.access_token || info?.accessToken || info?.success || info?.logged_in || info?.loggedIn || cookies.z_c0 || ['CONFIRMED', 'LOGIN_SUCCESS', 'SUCCESS', 'OK', 'LOGGED_IN'].includes(loginStatus))
}

function isQrExpired(info) {
  const loginStatus = String(info?.login_status || info?.loginStatus || '').toUpperCase()
  return info?.status === 2 || ['EXPIRED', 'QR_CODE_EXPIRED', 'LOGIN_EXPIRED'].includes(loginStatus)
}

function publicQrAttempt(attempt) {
  return {
    ok: true,
    id: attempt.id,
    status: attempt.status,
    message: attempt.message || '',
    expiresAt: attempt.deadline || 0,
    createdAt: attempt.createdAt,
    updatedAt: attempt.updatedAt,
    qrSvgDataUrl: attempt.qrSvgDataUrl || '',
    qrText: attempt.qrSvgDataUrl ? '' : (attempt.link || ''),
    riskUrl: attempt.riskUrl || '',
    riskProxyUrl: attempt.riskUrl ? `/zhihu-dashboard/qr-risk?id=${encodeURIComponent(attempt.id)}&url=${encodeURIComponent(attempt.riskUrl)}` : '',
    debug: attempt.debug || {},
  }
}
...
function zhihuRiskTargetFromParam(value, fallback = ZHIHU_RISK_CONTROL_URL) {
  try {
    const target = new URL(value || fallback)
    if (target.protocol !== 'https:' || !/(^|\.)zhihu\.com$/.test(target.hostname)) return new URL(fallback)
    return target
  } catch {
...
function riskProxyUrlFor(id, target) {
  const search = new URLSearchParams(target.search)
  search.set('id', id)
  return `/zhihu-dashboard/qr-risk-proxy${target.pathname}?${search.toString()}`
}

function proxiedZhihuUrl(pathname, searchParams) {
  const suffix = pathname.slice('/zhihu-dashboard/qr-risk-proxy'.length) || '/'
  const targetSearch = new URLSearchParams(searchParams)
  targetSearch.delete('id')
  const search = targetSearch.toString()
  return new URL(`${suffix}${search ? `?${search}` : ''}`, ZHIHU_HOME_URL)
}

function rewriteRiskHtml(html, id) {
  const proxyPrefix = '/zhihu-dashboard/qr-risk-proxy'
  const encodedId = encodeURIComponent(id)
  const withAttrs = html
    .replace(/(href|src|action)=(['"])https:\/\/(?:www\.)?zhihu\.com([^'"]*)\2/gi, (_m, attr, quote, path) => `${attr}=${quote}${proxyPrefix}${path}${path.includes('?') ? '&' : '?'}id=${encodedId}${quote}`)
    .replace(/(href|src|action)=(['"])(\/(?!\/)[^'"]*)\2/gi, (_m, attr, quote, path) => `${attr}=${quote}${proxyPrefix}${path}${path.includes('?') ? '&' : '?'}id=${encodedId}${quote}`)
  const script = `<script>(()=>{const id=${JSON.stringify(id)};const prefix='/zhihu-dashboard/qr-risk-proxy';const cookieUrl='/zhihu-dashboard/api/qr-login/risk-cookie';function prox(u){try{const x=new URL(u,location.href);if(x.origin===location.origin&&x.pathname.startsWith(prefix))return x.href;if(/(^|\\.)zhihu\\.com$/.test(x.hostname)){x.searchParams.set('id',id);return prefix+x.pathname+x.search+x.hash}return u}catch{return u}}const f=window.fetch;if(f)window.fetch=(u,o)=>f(prox(u),o);const open=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(m,u,...r){return open.call(this,m,prox(u),...r)};const d=Object.getOwnPropertyDescriptor(Document.prototype,'cookie');if(d&&d.set&&d.get){Object.defineProperty(document,'cookie',{get(){return d.get.call(document)},set(v){try{fetch(cookieUrl,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({id,cookie:String(v)})})}catch{}return d.set.call(document,v)}})}})();</script>`
  return withAttrs.replace(/<head(\s[^>]*)?>/i, (m) => `${m}${script}`)
}

function proxiedLocation(id, location, currentTarget) {
  if (!location) return ''
  try {
    const target = new URL(location, currentTarget)
    if (target.protocol !== 'https:' || !/(^|\.)zhihu\.com$/.test(target.hostname)) return ''
    return riskProxyUrlFor(id, target)
  } catch {
    return ''
  }
}

async function handleRiskProxy(req, res, url) {
  const id = url.searchParams.get('id') || ''
  const attempt = qrAttempts.get(id)
  if (!attempt) {
    writePage(res, '<!doctype html><meta charset="utf-8"><body>扫码登录已失效，请重新生成二维码。</body>')
    return
  }
  if (url.pathname === '/zhihu-dashboard/qr-risk') {
    const target = zhihuRiskTargetFromParam(url.searchParams.get('url'), attempt.riskUrl || ZHIHU_RISK_CONTROL_URL)
    res.writeHead(302, { location: riskProxyUrlFor(id, target), 'cache-control': 'no-store' })
    res.end()
    return
  }
  const target = proxiedZhihuUrl(url.pathname, url.searchParams)
  const headers = zhihuDesktopHeaders(target.href)
  headers.cookie = cookieHeaderFromMap(attempt.cookies)
  if (req.headers['content-type']) headers['content-type'] = req.headers['content-type']
  if (req.headers.accept) headers.accept = req.headers.accept
  const body = req.method === 'GET' || req.method === 'HEAD' ? undefined : await readRawBody(req)
  const upstream = await zhihuRequest(target.href, { method: req.method, headers, body, redirect: 'manual' }, attempt.cookies)
  logQrEvent(attempt, 'risk-proxy', { method: req.method, path: target.pathname, httpStatus: upstream.status, cookieCount: Object.keys(attempt.cookies).length, hasAuthCookie: Boolean(attempt.cookies.z_c0) })
  const redirected = proxiedLocation(id, upstream.headers.get('location'), target)
  if (redirected) {
    res.writeHead(upstream.status, { location: redirected, 'cache-control': 'no-store' })
    res.end()
    return
  }
  ...
  let out = buffer
  if (/text\/html/i.test(contentType)) out = Buffer.from(rewriteRiskHtml(buffer.toString('utf8'), id))
  attempt.debug = { ...(attempt.debug || {}), riskProxyStatus: upstream.status, hasAuthCookie: Boolean(attempt.cookies.z_c0) }
...
async function pollQrLogin(id) {
  const attempt = qrAttempts.get(id)
  if (!attempt) return { ok: false, error: '扫码登录已失效，请重新生成二维码。' }
  if (Date.now() > attempt.deadline && attempt.status !== 'succeeded') {
    attempt.status = 'expired'
    attempt.message = '二维码已过期，请重新生成。'
    attempt.updatedAt = Date.now()
    logQrEvent(attempt, 'expired', { hasAuthCookie: Boolean(attempt.cookies.z_c0) })
    return publicQrAttempt(attempt)
  }
  if (['succeeded', 'expired', 'failed', 'risk_control'].includes(attempt.status)) return publicQrAttempt(attempt)
  try {
    const res = await zhihuRequest(`${ZHIHU_QRCODE_URL}/${attempt.token}/scan_info`, { headers: zhihuLoginHeaders(attempt.cookies, ZHIHU_SIGNIN_URL, true) }, attempt.cookies)
    const raw = await readJsonSafe(res)
    const info = zhihuPayloadData(raw)
    info._httpStatus = res.status
    syncCookiesFromQrInfo(attempt.cookies, info)
    attempt.debug = qrDebugInfo(info, attempt.cookies)
    logQrEvent(attempt, 'poll', { ...attempt.debug, cookieCount: Object.keys(attempt.cookies).length })
    if (res.status === 403 || info?.error?.code === 40352 || info?.error?.need_login || info?.error?.needLogin) {
      attempt.riskUrl = info?.error?.redirect || 'https://www.zhihu.com/account/risk_control/'
      if (attempt.status === 'scanned' || attempt.seenScan === true) {
        attempt.status = 'risk_control'
        attempt.message = info?.error?.message || '知乎检测到网络环境异常，需要先完成官方验证。'
        logQrEvent(attempt, 'risk-control', { afterScan: true, riskProxyUrl: Boolean(attempt.riskUrl), errorCode: info?.error?.code || '' })
      } else {
        attempt.status = 'waiting'
        attempt.message = '知乎提示需要网络环境验证；二维码仍可先扫码；如手机端要求验证，请在共享会话验证页完成。'
        logQrEvent(attempt, 'risk-hint-before-scan', { riskProxyUrl: Boolean(attempt.riskUrl), errorCode: info?.error?.code || '' })
      }
    } else if (isQrExpired(info)) {
      attempt.status = 'expired'
      attempt.message = '二维码已过期，请重新生成。'
    } else if (isQrSuccess(info, attempt.cookies)) {
      await collectPostLoginCookies(attempt.cookies)
      if (attempt.cookies.z_c0) {
        saveZhihuSession(attempt.cookies)
        attempt.status = 'succeeded'
        attempt.message = '扫码登录成功，已保存只读会话。'
        logQrEvent(attempt, 'saved-session', { cookieCount: Object.keys(attempt.cookies).length, hasAuthCookie: true })
      } else {
        attempt.status = 'scanned'
        attempt.message = '手机端已确认，但尚未拿到登录 Cookie；继续轮询中。'
        logQrEvent(attempt, 'success-without-cookie', { cookieCount: Object.keys(attempt.cookies).length, hasAuthCookie: false })
      }
      attempt.debug = qrDebugInfo(info, attempt.cookies)
    } else if (info?.status === 1) {
      attempt.seenScan = true
      attempt.status = 'scanned'
      attempt.message = '已扫码，请在手机端确认登录。'
      logQrEvent(attempt, 'scanned', { scanStatus: info.status, hasAuthCookie: Boolean(attempt.cookies.z_c0) })
    } else {
      attempt.status = 'waiting'
      attempt.message = '等待扫码。'
    }
  } catch (error) {
    attempt.message = error instanceof Error ? error.message : String(error)
    logQrEvent(attempt, 'poll-error', { message: attempt.message })
  }
  attempt.updatedAt = Date.now()
  return publicQrAttempt(attempt)
}

async function continueQrLogin(id) {
  const attempt = qrAttempts.get(id)
  if (!attempt) return { ok: false, error: '扫码登录已失效，请重新生成二维码。' }
  if (attempt.status === 'risk_control') {
    attempt.status = attempt.seenScan ? 'scanned' : 'waiting'
    attempt.message = '已完成验证，继续检查扫码状态…'
    logQrEvent(attempt, 'continue-after-risk', { cookieCount: Object.keys(attempt.cookies).length, hasAuthCookie: Boolean(attempt.cookies.z_c0) })
  }
  return pollQrLogin(id)
}

function mergeRiskCookie(id, rawCookie) {
  const attempt = qrAttempts.get(id)
  if (!attempt) return { ok: false, error: '扫码登录已失效，请重新生成二维码。' }
  Object.assign(attempt.cookies, parseCookieAssignments(rawCookie))
  attempt.debug = { ...(attempt.debug || {}), riskCookieNames: Object.keys(attempt.cookies).filter((name) => !/^z_c0$/i.test(name)).slice(0, 8), hasAuthCookie: Boolean(attempt.cookies.z_c0) }
  logQrEvent(attempt, 'risk-cookie-js', { cookieCount: Object.keys(attempt.cookies).length, hasAuthCookie: Boolean(attempt.cookies.z_c0) })
  return { ok: true, hasAuthCookie: Boolean(attempt.cookies.z_c0) }
}

async function fetchSavedSessionItems(limit = 20) {
  const session = readZhihuSession()
  if (!session) return { ok: false, error: '尚未登录知乎只读会话' }
  return fetchBrowserSessionItems(cookieHeaderFromMap(session.cookies), limit)
}
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (pathname === '/zhihu-dashboard/qr-risk' || pathname.startsWith('/zhihu-dashboard/qr-risk-proxy')) {
            await handleRiskProxy(req, res, url)
            return
          }
          ...
          if (api === 'session-status' && (req.method === 'GET' || req.method === 'HEAD')) {
            writeJson(res, await sessionStatus(url.searchParams.get('verify') === '1'))
            return
          }
          if (api === 'qr-login/start' && req.method === 'POST') {
            const body = await readJsonBody(req)
            writeJson(res, await startQrLogin(String(body?.baseAttemptId || '')))
            return
          }
          if (api === 'qr-login/status' && (req.method === 'GET' || req.method === 'HEAD')) {
            const id = url.searchParams.get('id') || ''
            writeJson(res, url.searchParams.get('continue') === '1' ? await continueQrLogin(id) : await pollQrLogin(id))
            return
          }
          if (api === 'qr-login/risk-cookie' && req.method === 'POST') {
            const body = await readJsonBody(req)
            writeJson(res, mergeRiskCookie(String(body?.id || ''), String(body?.cookie || '')))
            return
          }
          if (api === 'qr-login/debug-log' && (req.method === 'GET' || req.method === 'HEAD')) {
            writeJson(res, { ok: true, events: publicQrLog() })
            return
          }
          if (api === 'qr-login/logout' && req.method === 'POST') {
            clearZhihuSession()
            writeJson(res, { ok: true })
            return
          }
          if (api === 'session-items' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const limit = clampLimit(body?.limit, 20, 50)
            writeJson(res, await fetchSavedSessionItems(limit))
            return
          }
