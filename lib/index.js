...
async function pollQrLogin(id) {
  ...
  try {
    ...
    const info = await readJsonSafe(res)
    ...
    if (res.status === 403 || info?.error?.code === 40352 || info?.error?.need_login || info?.error?.needLogin) {
      ...
      if (!attempt.cookies.z_c0) {
        await zhihuRequest(ZHIHU_ME_URL, { headers: zhihuLoginHeaders(attempt.cookies, ZHIHU_SIGNIN_URL, true) }, attempt.cookies).catch(() => null)
      }
      if (attempt.cookies.z_c0) {
