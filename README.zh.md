...
- **机会报告** —— 从浏览器会话、热榜/动态缓存、追踪新增、未读、工作台和研究项目聚合候选，按本地规则打机会分；可直接使用当前 DSH provider/model 生成开发机会报告，也可复制 Agent 报告提示兜底
- **只读浏览器会话** —— 高级模式可检测本机浏览器 profile、只读提取/验证 `zhihu.com` Cookie 并读取会话线索；Cookie 不进入模型、导出或日志，失败时可手动 Cookie 兜底
