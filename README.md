...
- **Data sources** — Access Secret remains the current main path; read-only Zhihu login state is being consolidated around the recommended QR-login path, while browser Cookie/profile reading has been removed from the user-facing UI because encrypted cookies are unreliable
